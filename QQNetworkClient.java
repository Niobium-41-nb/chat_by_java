 ...
 * 真正的网络通信QQ客户端
 */
public class QQNetworkClient extends JFrame {
    ...
    private String currentChatTarget = "群聊";
    
    public QQNetworkClient(String username, String password) {
    ...
    private void setupLayout() {
        ...
        JLabel statusLabel = new JLabel("服务器状态: ● 连接中...");
        ...
        friendList.addListSelectionListener(e -> {
            if (!e.getValueIsAdjusting()) {
                ...
                if (selectedFriend != null) {
                    currentChatTarget = selectedFriend;
        ...
        new Thread(() -> {
            try {
                ...
                while ((serverMessage = in.readLine()) != null) {
                    if (serverMessage.contains("请输入用户名")) {
                        break;
                    }
                }
                ...
                while ((serverMessage = in.readLine()) != null) {
                    if (serverMessage.contains("认证成功")) {
                        ...
                        break;
                    } else if (serverMessage.contains("认证失败")) {
    ...
    private void processServerMessage(String message) {
        ...
        if (message.startsWith("[私聊]")) {
            ...
            if (parts.length == 2) {
                ...
                // 添加到对应好友的聊天记录
                StringBuilder history = chatHistories.get(sender);
                if (history == null) {
                    history = new StringBuilder();
                    chatHistories.put(sender, history);
                }
                
                String timestamp = new java.text.SimpleDateFormat("HH:mm:ss").format(new Date());
                String formattedMessage = sender + " (" + timestamp + "): " + content + "\n";
                history.append(formattedMessage);
                
                // 如果当前正在与这个好友聊天，显示消息
                if (currentChatTarget.equals(sender)) {
            ...
            if (parts.length == 2) {
                ...
                // 添加到群聊记录
                StringBuilder history = chatHistories.get("群聊");
                if (history != null) {
                    String timestamp = new java.text.SimpleDateFormat("HH:mm:ss").format(new Date());
                    String formattedMessage = sender + " (" + timestamp + "): " + content + "\n";
                    history.append(formattedMessage);
                    ...
                    // 如果当前正在群聊，显示消息
                    if (currentChatTarget.equals("群聊")) {
                        chatArea.append(formattedMessage);
                        chatArea.setCaretPosition(chatArea.getDocument().getLength());
                    }
                }
    ...
    private void sendMessage() {
        ...
        if (currentChatTarget.equals("群聊")) {
            ...
            formattedMessage = "@" + currentChatTarget + " " + message;
        }
        ...
        StringBuilder history = chatHistories.get(currentChatTarget);
        if (history != null) {
